...
class LoggingUpdate(object):
    def update_stats(self, stats):
        LOG.debug("Amphora %s A10, update stats %s",
                  self.__class__.__name__, stats)
        self.stats = stats

    def update_health(self, health):
        LOG.debug("Amphora %s A10, update health %s",
                  self.__class__.__name__, health)
        self.health = health


class ACOSApi(object):

    def __init__(self):
        super(ACOSApi, self).__init__()
        self.amphoraconfig = {}
    ...
    def update_amphora_listeners(self, listeners, amphora_index,
        ...
        for listener in listeners:
            LOG.debug("Amphora A10 driver update_amphora_listeners, "
    ...
    def update(self, listener, vip):
        LOG.debug("Amphora %s A10, update listener %s, vip %s",
    ...
    def stop(self, listener, vip):
        LOG.debug("Amphora %s A10, stop listener %s, vip %s",
    ...
    def start(self, listener, vip):
        LOG.debug("Amphora %s A10, start listener %s, vip %s",
                  self.__class__.__name__,
                  listener.protocol_port, vip.ip_address)
        self.amphoraconfig[(listener.protocol_port,
    ...
    def delete(self, listener, vip):
        LOG.debug("Amphora %s A10, delete listener %s, vip %s",
                  self.__class__.__name__,
                  listener.protocol_port, vip.ip_address)
        self.amphoraconfig[(listener.protocol_port,
    ...
    def post_vip_plug(self, amphora, load_balancer, amphorae_network_config):
        LOG.debug("Amphora %s A10, post vip plug load balancer %s",
    ...
    def upload_cert_amp(self, amphora, pem_file):
        LOG.debug("Amphora %s A10, upload cert amphora %s,with pem fle %s",
