...
class TestHandlerServerTasks(base.BaseTaskTestCase):
    ...
    def test_MemberCreate_execute_create_with_flavor(self):
        ...
        member_port_count_ip = 1
        flavor = repo.FlavorProfileRepository()
        flavor.flavor_data = '{"server":{"conn_limit":65535, "conn_resume": 5000}}'
        expect_axapi_arg = {'server': {'conn-limit': 65535, 'conn-resume': 5000}}
        ...
        self.client_mock.slb.server.create.assert_called_with(
            ...
            server_templates=mock.ANY, axapi_args=expect_axapi_arg)

    def test_MemberCreate_execute_create_with_regex_flavor(self):
        mock_create_member = task.MemberCreate()
        member_port_count_ip = 1
        flavor = repo.FlavorProfileRepository()
        flavor.flavor_data = '{"server": {"name-expressions": [{"regex": "-limited-",'
        flavor.flavor_data += '"json": {"conn-limit":65535, "conn-resume": 5000}}]}}'
        expect_axapi_arg = {'server': {'conn-limit': 65535, 'conn-resume': 5000}}
        mock_create_member.CONF = self.conf
